...
botaoBuscar.addEventListener("click", async (e) => {
    ...
    informacoes.innerHTML = `
        ...
        <select class="input-atualizar" name="estado" id="estadoObjeto">
            <option value="Ótimo" ${objetoEncontrado.estado === "Ótimo" ? "selected" : ""}>Ótimo</option>
            <option value="Bom" ${objetoEncontrado.estado === "Bom" ? "selected" : ""}>Bom</option>
            <option value="Ruim" ${objetoEncontrado.estado === "Ruim" ? "selected" : ""}>Ruim</option>
        </select><br><br>

       <div class="lugares">
        <label class="descrição-input" for="piso">Piso</label><br>
        <select class="input-atualizar" id="piso">
            <option value="piso1" ${objetoEncontrado.piso === "piso1" ? "selected" : ""}>Piso 1</option>
            <option value="piso2" ${objetoEncontrado.piso === "piso2" ? "selected" : ""}>Piso 2</option>
            <option value="Garagem" ${objetoEncontrado.piso === "Garagem" ? "selected" : ""}>Garagem</option>
        </select>
      </div><br>

      <div class="lugares">
        <label class="descrição-input" for="local">Nome do local:</label>
        <input class="input-atualizar" type="text" id="local" value="${objetoEncontrado.local || ''}">
      </div>
